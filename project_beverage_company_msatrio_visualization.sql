...
select split(InventoryId, '_')[safe_offset(0)] as store_name, sum(SalesDollars) as total_penjualan
from sales
group by 1
order by 2 desc

-- -- penjualan terbanyak per kota
-- select split(InventoryId, '_')[safe_offset(1) as city, sum(SalesDollars) as total_penjualan  
-- from sales
-- group by 1
-- order by 2 desc
